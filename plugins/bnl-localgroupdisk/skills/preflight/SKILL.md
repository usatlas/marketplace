...
# LOCALGROUPDISK migration pre-flight checks

Run all prerequisite checks before migrating files to BNL-OSG2_LOCALGROUPDISK.

## Checks

Run all checks and report a summary table.
...
Record the `account` name — this is the Rucio scope prefix (e.g., `user.<account>`).
...
- VOMS attributes include `/atlas/usatlas` — **required for LOCALGROUPDISK quota**
- If missing `/atlas/usatlas`: user must request membership via ATLAS IAM
  (`https://atlas-auth.cern.ch/`), then `voms-proxy-init -voms atlas:/atlas/usatlas`
...
Note the exact hyphen/underscore positions — they matter.
...
- LOCALGROUPDISK should show a limit (default 50 TB). If missing, the user
  is not in `/atlas/usatlas` VOMS group.
...
User files land under `/pnfs/usatlas.bnl.gov/LOCALGROUPDISK/rucio/user/<account>/`.
