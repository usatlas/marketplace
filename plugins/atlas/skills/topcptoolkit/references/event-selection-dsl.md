...
The `selectionCuts` value in each `EventSelection:` block is a mini-language: one
keyword per line, processed top-to-bottom. All pT and mass thresholds are in
**MeV**.
...
not used as event filters; they only produce a decision flag that other regions can
`IMPORT`.
...
The resulting output branches are `pass_SUBpresel_%SYS%`, `pass_ejets_%SYS%`, and
`pass_mujets_%SYS%`.
